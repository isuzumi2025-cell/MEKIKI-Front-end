...
class UnifiedApp(ctk.CTk):
    ...
    def _run_ai_analysis_mode(self):
        ...
        try:
                ...
                if best_match:
                    ...
                    class ParaSyncPair:
                        def __init__(self, web_p: Paragraph, pdf_p: Paragraph, sim: float):
                            # カラム情報をIDに含める
                            self.web_id = f"Col{web_p.column}-{web_p.id}"
                            self.pdf_id = f"Col{pdf_p.column}-{pdf_p.id}"
                            self.similarity = sim
                ...
                if not found:
                    class UnmatchedPair:
                        def __init__(self, p: Paragraph, source: str):
                            self.web_id = f"Col{p.column}-{p.id}" if source == 'web' else ""
                            self.pdf_id = "" if source == 'web' else f"Col{p.column}-{p.id}"
                            self.similarity = 0.0
            ...
            class ParaRegion:
                def __init__(self, p: Paragraph, source: str):
                    self.area_code = f"Col{p.column}-{p.id}"
                    self.text = p.text
                    self.rect = p.bbox
                    self.similarity = 0.0
                    self.column = p.column

            web_regions = [ParaRegion(p, 'web') for p in web_paragraphs]
            pdf_regions = [ParaRegion(p, 'pdf') for p in pdf_paragraphs]
