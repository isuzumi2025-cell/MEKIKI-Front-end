...
class SpreadsheetPanel(ctk.CTkFrame):
    ...
    def _create_row(self, index: int, pair):
        ...
        # ★ PAGE-AWARE: ページリストを使用してサムネイル作成
        web_thumb = self._create_page_aware_thumbnail(web_region, self.web_pages, self.web_image, "web")
        if web_thumb:
        ...
        # ★ PAGE-AWARE: ページリストを使用してサムネイル作成
        pdf_thumb = self._create_page_aware_thumbnail(pdf_region, self.pdf_pages, self.pdf_image, "pdf")
        if pdf_thumb:
    ...
    def _create_page_aware_thumbnail(self, region, pages_list, fallback_image, source: str):
        """★ PAGE-AWARE: region.page_idで正しいページ画像を選択してサムネイル作成
        
        Args:
            region: EditableRegion with rect and page_id
            pages_list: List of page dicts with 'image' key
            fallback_image: ステッチ画像（フォールバック用）
            source: "web" or "pdf"
        """
        if not region:
            log_diagnostic(f"[PageThumb] FAIL: region is None")
            return None
            
        bbox = getattr(region, 'rect', None)
        if not bbox:
            log_diagnostic(f"[PageThumb] FAIL: region.rect is None")
            return None
        ...
        if pages_list and len(pages_list) >= page_id and page_id > 0:
            ...
            log_diagnostic(f"[PageThumb] Using page {page_id} image: {page_image.size if page_image else 'None'}")
        
        # フォールバック: ページリストがない場合はステッチ画像を使用
        if not page_image:
            log_diagnostic(f"[PageThumb] Fallback to stitch image for page {page_id}")
            page_image = fallback_image
        
        if not page_image:
            log_diagnostic(f"[PageThumb] FAIL: No image available")
            return None
        
        # サムネイル作成（ページ相対座標を使用）
        return self._create_thumbnail(page_image, bbox)
