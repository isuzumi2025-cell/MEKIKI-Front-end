...
class MicroView(ctk.CTkFrame):
    ...
    def _build_llm_panel(self, parent):
        """LLM分析結果表示パネル - Genius Edition"""
        # 区切り線
        ctk.CTkFrame(parent, height=1, fg_color="#3F3F46").pack(fill="x", padx=10, pady=5)

        llm_frame = ctk.CTkFrame(parent, fg_color="#18181B", height=200) # Zinc-900
        ...
        ctk.CTkLabel(
            ...
            font=("Arial", 12, "bold"),
            text_color="#C084FC" # Purple-400
        ).pack(side="left")
        
        self.llm_status = ctk.CTkLabel(header, text="", font=("Arial", 10), text_color="#71717A")
