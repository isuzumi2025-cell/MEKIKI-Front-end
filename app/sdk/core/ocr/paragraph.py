...
class ParagraphDetector:
    ...
    def detect_from_image(self, image_path: str) -> List[Paragraph]:
        ...
        ImageSegmentクラスを使用してリサイズと座標変換を一元管理
        ...
        try:
            from app.core.ocr_engine import OCREngine
            from app.sdk.core.segment import ImageSegment, BBox
        ...
        # ImageSegmentを使用してリサイズを処理
        segment = ImageSegment.from_path(image_path, max_long_edge=4096, segment_id=Path(image_path).stem)
        
        try:
            # OCRパスを取得（リサイズ済みの場合は一時ファイル）
            ocr_path = segment.get_temp_path() if segment.is_resized else image_path
        ...
        for block in result.get("blocks", []):
            ...
            if text:  # 空でなければ全て取得
                # ImageSegmentの座標変換を使用
                if segment.is_resized:
                    resized_bbox = BBox.from_list(bbox)
                    original_bbox = segment.bbox_to_original(resized_bbox)
                    bbox = original_bbox.to_list()
