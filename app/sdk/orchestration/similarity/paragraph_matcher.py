...
class EnhancedParagraphMatcher:
    ...
    def _compute_similarity_matrix(
        ...
        # 空間フィルタリング用のヘルパー
        def is_vertically_aligned(r1, r2, tolerance=0.2):
            """Y座標が許容範囲内で重なっているか判定"""
            # rect = [x1, y1, x2, y2]
            bbox1 = self._get_bbox(r1)
            bbox2 = self._get_bbox(r2)
            
            if not bbox1 or not bbox2:
                return True # 座標なしはフィルタしない
            ...
            # 平均高さ
            avg_h = (h1 + h2) / 2
            if avg_h == 0: return True
            
            # 垂直距離が平均高さの特定倍数以内か
            # tolerance=0.2 means very strict, 2.0 means loose
            # ヘッダー(上)とフッダー(下)をマッチさせないためには、
            # ページ全体の高さを考慮するか、または相対位置を見る必要がある
            # ここではシンプルに「Y座標の差が一定以内」を見る
            
            diff = abs(cy1 - cy2)
            # ページ高さ(推定)に対する割合でなく、ピクセル差で見る場合
            # 異なる解像度(Web vs PDF)だと危険だが、Global座標系は統一されている前提
            
            # より安全なアプローチ:
            # 「WebとPDFで解像度が違う可能性がある」ため、
            # 絶対座標比較は危険。しかし、Coordinate Normalization後はGlobal Coordinatesになっているはず。
            # 安全のため、少し緩めのフィルタにする (Y差が1000px以上ならマッチしない等)
            
            limit = 2000 # 仮: 2000px以上離れていたらマッチしない
            if diff > limit:
                return False
        ...
        for i, web in enumerate(web_regions):
            ...
            for j, pdf in enumerate(pdf_regions):
                # ★ 空間フィルタリング
                # Global Coordinate Systemが統一されているので、Y座標乖離チェックが有効
                if not is_vertically_aligned(web, pdf):
                    score = 0.0
                    detail = MatchDetail()
                    skipped_count += 1
                else:
        ...
        print(f"  ⚡ 空間フィルタリング: {skipped_count} ペアをスキップ")
