...
"""
    ...
    def analyze_image(
        ...
        """
        # ソースの確認
        if not image and (not image_path or not Path(image_path).exists()):
            print(f"⚠️ 画像ソースが見つかりません: {image_path}")
            return []
        ...
        try:
            # OCR実行 (パスまたは画像オブジェクトを渡す)
            source = image if image else image_path
            source_name = Path(image_path).name if image_path else "In-memory Image"
            
            print(f"🔍 OCR実行中: {source_name}")
            result = self.ocr_engine.detect_document_text(source)
            
            if not result:
                print(f"⚠️ OCR結果が取得できませんでした: {source_name}")
                return []
            ...
            print(f"✅ {len(areas)} エリア検出: {source_name}")
            return areas
    ...
    def load_page(
        ...
        title: str = "",
        image: Optional[Image.Image] = None,
        areas: Optional[List[DetectedArea]] = None
    ) -> Optional[PageData]:
        """
        画像/PDFからPageDataを作成し、リストに追加する
        
        Args:
            image_path: 画像ファイルのパス (optional)
            source_type: "web" or "pdf"
            source_id: URL (Web) or Filename (PDF)
            page_num: ページ番号 (PDF)
            title: ページタイトル
            image: PIL Imageオブジェクト (指定された場合、ファイル読み込みをスキップ)
            areas: 検出済みエリアリスト (指定された場合、OCRをスキップ)
        
        Returns:
            作成されたPageData (失敗時はNone)
        """
        try:
            print(f"📄 [Page Load] 読み込み開始: {source_id} (Page {page_num})")
            
            # 画像の準備
            pil_image = image
            if not pil_image and image_path:
                pil_image = Image.open(image_path)
                pil_image.load()
            
            if not pil_image:
                print("❌ [Page Load] 画像が指定されていません")
                return None
                
            # エリアの準備 (指定がない場合はOCR実行)
            final_areas = areas
            if final_areas is None:
                if image_path or pil_image:
                    final_areas = self.analyze_image(
                    ...
                    print("⚠️ [Page Load] 画像ソースがないためOCRを実行できません")
                    final_areas = []
            
            # 全文テキストを結合
            full_text = "\n".join([area.text for area in final_areas])
            ...
            page_data = PageData(
                ...
                source_id=source_id,
                page_num=page_num,
                title=title or (Path(image_path).name if image_path else source_id),
                text=full_text,
                image=pil_image,
                image_path=image_path,
                areas=final_areas
            ...
            # リストに追加
            if source_type == "web":
                self.web_pages.append(page_data)
            elif source_type == "pdf":
                self.pdf_pages.append(page_data)
            
            print(f"✅ [Page Load] 完了: {len(final_areas)} areas, {len(full_text)} chars")
            return page_data
