...
class AdvancedComparisonView(ctk.CTkFrame):
    ...
    def _build_ui(self):
        ...
        # Simulation Button (New)
        ctk.CTkButton(
            toolbar, text="🧪 Simulate", width=80, fg_color="#673AB7",
            command=self._open_match_simulator
        ).pack(side="left", padx=5)

        # ✨ 類似パターン検出 (Template Propagation)
        ctk.CTkButton(
            toolbar, text="✨ 類似検出", width=100, fg_color="#E91E63",
            command=lambda: self._open_region_editor('web') # Default to Web? Or alert?
        ).pack(side="left", padx=5)
        ...
        # ⚖️ マトリクスボタン
        ctk.CTkButton(
            toolbar, text="⚖️ Matrix", width=80, fg_color="#673AB7",
