...
def cluster_paragraphs(
    ...
    for elem in elements:
        ...
        for cluster in clusters:
            ...
            # クラスタの代表要素を取得
            rep_elem = cluster[0]
            rep_bbox = rep_elem.get("bbox", {})
            rep_center_x = (rep_bbox.get("x1", 0) + rep_bbox.get("x2", 0)) / 2
            rep_center_y = (rep_bbox.get("y1", 0) + rep_bbox.get("y2", 0)) / 2
            rep_width = rep_bbox.get("x2", 0) - rep_bbox.get("x1", 0)
            rep_height = rep_bbox.get("y2", 0) - rep_bbox.get("y1", 0)
            rep_size = rep_width * rep_height
            
            # 距離チェック
            distance = math.sqrt(
                (elem_center_x - rep_center_x) ** 2 + (elem_center_y - rep_center_y) ** 2
            )
            
            # 整列チェック（水平方向）
            align_diff = abs(elem_center_x - rep_center_x)
            
            # サイズ類似度チェック
            size_ratio = min(elem_size, rep_size) / max(elem_size, rep_size) if max(elem_size, rep_size) > 0 else 0
