...
class AdvancedComparisonView(ctk.CTkFrame):
    ...
    def _on_pdf_canvas_configure(self, event):
        ...
        if getattr(self, '_display_in_progress', False):
            print(f"[Configure] PDF skipped (display_in_progress)")
            return
        if hasattr(self, 'pdf_image') and self.pdf_image and event.width > 50:
            print(f"[Configure] PDF resize triggered, width={event.width}")
            # デバウンス: 前回の呼び出しをキャンセル
            if hasattr(self, '_pdf_resize_job') and self._pdf_resize_job:
                self.after_cancel(self._pdf_resize_job)
            # ★ 画像描画後にリージョンも再描画（Case2修正: AI分析後の画像消失問題）
            def _redisplay_pdf():
                self._display_image(self.pdf_canvas, self.pdf_image)
                if self.pdf_regions:
                    self._redraw_regions()
            self._pdf_resize_job = self.after(100, _redisplay_pdf)

    def _on_web_canvas_configure(self, event):
        """Webキャンバスリサイズ時に画像を再描画"""
        # ★ 描画中は再描画をスキップ（干渉防止）
        if getattr(self, '_display_in_progress', False):
            print(f"[Configure] Web skipped (display_in_progress)")
            return
        if hasattr(self, 'web_image') and self.web_image and event.width > 50:
            print(f"[Configure] Web resize triggered, width={event.width}")
            # デバウンス: 前回の呼び出しをキャンセル
    ...
    def _display_image(self, canvas: tk.Canvas, image: Image.Image):
        ...
        try:
            ...
            print(f"[_display_image] canvas_width={canvas_width}, image={image.size}")
            ...
            print(f"[_display_image] Canvas items: total={len(items)}, image_tags={len(image_items)}")
    ...
    def _redraw_regions(self):
        ...
        try:
            ...
            for canvas, regions, source in [
                ...
                print(f"[_redraw_regions] {source}: scale=({scale_x:.4f},{scale_y:.4f}), offset=({offset_x},{offset_y})")
