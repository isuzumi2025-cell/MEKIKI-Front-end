# Visual Sitemap Generator ‚Äî Windows Runbook (Local Verification)
...
## 0) What ‚ÄúPASS‚Äù means (minimum bar)
...
```
If missing, install per your project‚Äôs dependency management (pip/uv/poetry).

---

## 3) Open ‚Äúanother PowerShell‚Äù (second terminal)
Keep Uvicorn running in one PowerShell window, and run checks in another window.

Ways to open a second window:
- Start menu ‚Üí type `powershell` ‚Üí Enter
...
```
Also confirm Uvicorn‚Äôs ‚Äúrunning on ‚Ä¶‚Äù line and match the port.
...
## Verify (one-command)
é¿çsÉRÉ}ÉìÉh:
powershell -ExecutionPolicy Bypass -File C:\Users\raiko\OneDrive\Desktop\26\OCR\verify\run_verify_all.ps1

Artifacts:
- C:\Users\raiko\OneDrive\Desktop\26\OCR\verify\result_setup.txt
- C:\Users\raiko\OneDrive\Desktop\26\OCR\verify\result_jobs.txt
- C:\Users\raiko\OneDrive\Desktop\26\OCR\verify\result_p004.txt
