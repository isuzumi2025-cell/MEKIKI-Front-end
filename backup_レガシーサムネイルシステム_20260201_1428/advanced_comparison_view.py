...
class AdvancedComparisonView(EditMixin, SelectionMixin, ctk.CTkFrame):
    ...
    def _refresh_inline_spreadsheet(self):
        ...
        # ★ ROOT CAUSE FIX: regions use stitch coords, so use stitch images
        # Web側: web_pagesから全ページをステッチ
        if hasattr(self, 'web_pages') and self.web_pages:
            web_img = self._stitch_pages_vertically([p['image'] for p in self.web_pages])
        
        # PDF側: 既存のステッチ画像を使用
        if hasattr(self, 'pdf_stitched_groups') and self.pdf_stitched_groups:
            # 全グループを連結（またはcurrent groupを使用）
            pdf_img = self.pdf_stitched_groups[getattr(self, 'current_pdf_group_idx', 0)]['image']
    ...
    def _run_ocr_analysis(self):
        ...
        try:
            ...
            if self.web_image:
                ...
                web_page_offsets = [0]  # 各ページのY開始位置 (累積)
                if hasattr(self, 'web_pages') and self.web_pages:
                    cumulative_y = 0
                    for page in self.web_pages:
                        img = page.get('image')
                        if img:
                            web_page_offsets.append(cumulative_y + img.height)
                            cumulative_y += img.height
                    print(f"[Phase44] web_page_offsets: {web_page_offsets}")
    ...
    def _run_ocr_analysis(self):
        ...
        try:
            ...
            if getattr(self, 'web_image', None):
                 self._safe_status("🔥 Hybrid OCR: Web画像を解析中...")
            ...
            if getattr(self, 'pdf_image', None):
                ...
                if not pdf_embedded_success:
                    self._safe_status("🔥 Hybrid OCR: PDF画像を解析中...")
    ...
    def _process_ocr_result(self, result, source):
        ...
        for i, block in enumerate(blocks):
             ...
             if source == "web":
                 ...
                 # ★ ROOT CAUSE FIX: ステッチ座標を保持（ページ相対変換しない）
                 # サムネイル作成時にステッチ画像を使用するため
                 r = EditableRegion(
                     id=seq,
                     rect=list(rect),  # ステッチ座標をそのまま使用
                     text=text,
                     area_code=f"P{page_num}-{seq}",  # P{page}-{seq}形式
                     sync_number=None,
                     similarity=0.0,
                     source=source,
                     page_id=page_num  # ページ番号は参照用に保持
                 )
             else:
                 # PDF: スティッチモード（page_id=-1）
                 r = EditableRegion(
                     id=seq,
                     rect=list(rect),
                     text=text,
